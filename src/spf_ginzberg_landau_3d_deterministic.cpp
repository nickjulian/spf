...
int main( int argc, char* argv[])
{
   ...
   SPF_NS::free_energy_gl F_gl(
            ...
            D_T,
            0.0001 // rk_dt, used by marcus version, not this version
         );
      ...
      for (size_t ii=1; ii < Nx_local +1; ++ii) // loop over non-ghosts
         for ( size_t jj=0; jj < Ny; ++jj)
            for ( size_t kk=0; kk < Nz; ++kk)
            {
               ...
               // evaluate deterministic fluxes (walkers) to neighbor cells
               SPF_NS::conserved_jump_flux_deterministic( 
